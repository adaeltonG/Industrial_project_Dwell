...
export default function CheckoutPage() {
  ...
  return <><Header /><main className="page shopping-page">
    {receipt ? <section className="shopping-receipt" aria-labelledby="receipt-title"><CheckCircle2 size={48} aria-hidden="true" /><p className="shopping-eyebrow">{receipt.reference}</p><h1 id="receipt-title">Your demo is complete.</h1><p>No payment was taken. No order was placed, and nothing will be delivered.</p><div className="shopping-receipt__items"><h2>Simulated order</h2>{receipt.items.map(({ product, quantity }) => <div className="checkout-item" key={product.id}><ProductImage product={product} className="checkout-item__image" /><div><h3>{product.title}</h3><p>Quantity: {quantity}</p></div><strong>{formatPrice((toMinorUnits(product.price) * quantity / 100).toFixed(2), product.currency)}</strong></div>)}<p className="shopping-receipt__total">Demo total <strong>{formatPrice((receipt.total / 100).toFixed(2), receipt.currency)}</strong></p></div><p>Your delivery details have not been saved.</p><Link href="/products" className="btn btn--primary">Continue shopping</Link></section> : <>
      <Link href="/cart" className="shopping-back">← Back to cart</Link><div className="shopping-heading"><p className="shopping-eyebrow">A preview, without the payment</p><h1>Demo checkout</h1><p>Try the checkout experience. No card details, payment or real order.</p></div>
      {!ready ? <p role="status">Loading your cart…</p> : !items.length ? <section className="shopping-empty"><h2>Your cart is empty.</h2><p>Add a product to try the checkout simulation.</p><Link href="/products" className="btn btn--primary">Explore products</Link></section> : <div className="shopping-layout">
        <form className="checkout-form" onSubmit={simulateCheckout}><h2>Demo delivery details</h2><p>Use sample details to preview delivery. These details are used only on this page and are not saved or sent to our server.</p><fieldset disabled={pending}><legend className="sr-only">Demo delivery details</legend><label>Full name<input name="name" autoComplete="off" placeholder="Alex Green" maxLength={120} required /></label><label>Email address<input name="email" type="email" autoComplete="off" placeholder="alex@example.com" maxLength={254} required /></label><label>Delivery address<input name="address" autoComplete="off" placeholder="12 Garden Lane, London" maxLength={300} required /></label><label>Postcode<input name="postcode" autoComplete="off" placeholder="SW1A 1AA" maxLength={20} required /></label></fieldset>{error ? <p className="form-error" role="alert">{error} <Link href="/cart">Review cart</Link></p> : null}<button className="btn btn--primary" disabled={pending} type="submit">{pending ? "Checking products…" : "Complete demo checkout"}</button><p className="checkout-assurance" role="status">{pending ? "Confirming current prices and availability. Please keep this page open." : "No payment will be taken."}</p></form>
