...
async function main() {
  ...
  for (const category of categories) {
    const record = await prisma.category.upsert({ where: { slug: category.slug }, update: category, create: category });
    categoryIds.set(category.slug, record.id);
  }

  const vendorIds = new Map<string, string>();
  for (const vendor of vendors) {
    const record = await prisma.vendor.upsert({ where: { slug: vendor.slug }, update: vendor, create: vendor });
